package CSU_Yunlu_2020.standard;
...
public class BuildingInfo {
	...
	private static List<Building> neighbours;

	public static enum BUILDING_CONDITION {
		/**
		 * Building not on fire, but its temperature is greater than a certain
		 * value which is 25.
		 */
		BUILDING_WARM,
		...
		BUILDING_ON_FIRE,
		/**
		 * Building with Fireyness WATER_DAMAGE, MINOR_DAMAGE, MODERATE_DAMAGE
		 * or SEVERE_DAMAGE.
		 */
		...
		BUILDING_COLLAPSED,
		/**
		 * BUilding not o fire and with temperature less than a certain value
		 * which is 25.
		 */
		BUILDING_UNBURNT
	};
	...
	private static BUILDING_CONDITION predicateBuildingCondition(Building target) {
		...
		case UNBURNT:
			...
			if (!(target instanceof Refuge) && !(target instanceof PoliceOffice) && !(target instanceof AmbulanceCentre)
		...
		case SEVERE_DAMAGE:
			...
			else if (!(target instanceof Refuge) && !(target instanceof PoliceOffice)
					&& !(target instanceof AmbulanceCentre) && !(target instanceof FireStation))
				return BUILDING_CONDITION.BUILDING_WARM;
			break;
		case BURNT_OUT:
	...
	/**
	 * If a <b>Building</b> is on fire, then the buildings arounds this building
	 * may catch a fire too. So those building are in dangerous.
	 * <p>
	 * This method get all dangerous building in a certain range around a fired
	 * building. Here the radius of this range is 7m which is defined by
	 * <b>MAX_DIST_NEAR_BUILDINGS</b>
	 * <p>
	...
	public ArrayList<Building> inDangerNeighbors() {
		//
		if (building.getFieryness() == 4 || building.getFieryness() == 0 || !building.isFierynessDefined()) {
			return null;
		}
		...
		for (Building b : neighbours) {
			if (b.getFieryness() == 0) { /// all gas stations are dangerous
				if (!(b instanceof Refuge) && !(b instanceof AmbulanceCentre) && !(b instanceof PoliceOffice)
	 ...
	 */
	public List<Building> getNeighbours(WorldInfo world) {
		if (neighbours == null) {
			neighbours = findNearBuildings(world, building, MAX_DIST_NEAR_BUILDINGS);
	...
	public List<Building> findNearBuildings(WorldInfo world, Building centerbuilding, int distance) {
		...
		allObjects = world.getObjectsInRange(centerbuilding, radius);
	...
	/**
	 * Determines whether a specified building is the neighbour of this
	 * building.
	 * 
	 * @param building
	 *            &nbsp;the building needs determines
	 * @param world
	 *            &nbsp;the world model
	 * @return true when the specified building is this building's neighbour,
	 *         false otherwise
	 */
	public boolean isNeighbourBuilding(Building building, WorldInfo world) {
		if (neighbours == null) {
	 ...
	 * 
	 * private int latestTemperature; private int temperatureUpdateTime;
	 * if(building.isTemperatureDefined()) { latestTemperature =
	 * building.getTemperature(); } else { latestTemperature = 0; }
	 * 
	 * public int getLatestTemperature() { return latestTemperature; }
	 * 
	 * public void setLatestTemperature(int latestTemperature) {
	 * this.latestTemperature = latestTemperature; }
	 * 
	 * ---Determines whether this Building is heating. public boolean
	 * isHeating() { if (building.isFierynessDefined() &&
	 * building.getFierynessEnum() != Fieryness.BURNT_OUT) { int d; boolean
	 * heating;
	 * 
	 * d = getTemperatureDifference(); heating = (d > MIN_HEATING_VALUE); return
	 * heating; } else { return false; } }
	 * 
	 * --- Get the heating speed of this building. public double
	 * getHeatingSpeed(int timeStep) { int timeDifference; timeDifference =
	 * timeStep - temperatureUpdateTime; if (timeDifference > 0) { int
	 * temperatureDifference; double r;
	 * 
	 * temperatureDifference = getCurrentTemperature() - latestTemperature; r =
	 * (double) temperatureDifference / timeDifference; return r; } else {
	 * return 0; } }
	 * 
	 * 
	 * --- Get current temperature of this building. public int
	 * getCurrentTemperature() { if (building.isTemperatureDefined()) { return
	 * building.getTemperature(); } else { return 0; } }
	 * 
	 * public int getTemperatureDifference() { int temperature, difference;
	 * 
	 * temperature = getCurrentTemperature(); difference = temperature -
	 * latestTemperature; return difference; }
	 * 
	 * public void updateTemperature(int timeStep) { if
	 * (building.isTemperatureDefined()) { latestTemperature =
	 * building.getTemperature(); temperatureUpdateTime = timeStep; } }
	 */
}
