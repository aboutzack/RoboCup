package CSU_Yunlu_2020.standard.simplePartition;
...
public class RegionModel {
	...
	protected WorldInfo model;
	protected ScenarioInfo scenarioInfo;

	public RegionModel(AgentInfo agentInfo, WorldInfo model, ScenarioInfo scenarioInfo, GroupingType type) {
		int teamSize;
	...
	// calculate regions;get regions
	// info:row,column,entities(building,refuge,all center)
	public EntityRegion[][] calculateRegions(int row, int column) {
		...
		entities = model.getEntitiesOfType(StandardEntityURN.BUILDING, StandardEntityURN.ROAD, StandardEntityURN.REFUGE,
				StandardEntityURN.AMBULANCE_CENTRE, StandardEntityURN.POLICE_OFFICE, StandardEntityURN.FIRE_STATION,
		...
		for (StandardEntity entity : entities) {
			...
			c = getColumn((int) model.getBounds().getMinX(), (int) model.getBounds().getWidth(), column, point.x); // to
																													// determine
																													// which
																													// column
																													// the
																													// point
																													// is.
			r = getColumn((int) model.getBounds().getMinY(), (int) model.getBounds().getHeight(), row, point.y); // to
																													// determine
																													// which
																													// row
																													// the
	...
	protected List<Line> getGridLines(int n) {
		...
		double dy = h / (n + 1);
		for (int i = 0; i < n; i++) {
			lines.add(new Line(bounds.getMinX(), bounds.getMinY() + (i + 1) * dy, bounds.getMaxX(),
					bounds.getMinY() + (i + 1) * dy));
			lines.add(new Line(bounds.getMinX() + (i + 1) * dx, bounds.getMinY(), bounds.getMinX() + (i + 1) * dx,
					bounds.getMaxY()));
		}
		return lines;
	}
	...
	protected int getTotalBuildingArea() {
		List<Building> buildings = getEntitiesOfType(Building.class, StandardEntityURN.BUILDING);
		int totalArea = 0;
	...
	public <T extends StandardEntity> List<T> getEntitiesOfType(Class<T> c, StandardEntityURN urn) {
		Collection<StandardEntity> entities = model.getEntitiesOfType(urn);
	...
	protected List<Building> getBuildings() {
		return getEntitiesOfType(Building.class, StandardEntityURN.BUILDING);
	}
