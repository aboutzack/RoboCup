...
public class CSURoadHelper {
	...
	/**
	 * When {@link CSURoadHelper#pfClearLines} is null, the roadCenterLine is null,
	 * too.
	 */
	 ...
	 * For entrance only.
	 * 
	 * @return true when this entrance is need to clear. 140824 true -- dont
	 *         need ,false -- need
	 */
	...
	public boolean isPassable() {
		if (isAllEdgePassable() || isOneEdgeUnpassable()) {
			...
			/*
			 * for (CSUBlockade next : getCsuBlockades()) { if
			 * (next.getPolygon().contains(selfRoad.getX(), selfRoad.getY()))
			 * return false; }
			 */
			// return true;
	...
	/**
	 * The method {@link CSURoadHelper#getPfClearLine(CSURoadHelper)} should be invoked
	 * somewhere before using of this method.
	 * 
	 * @return the center line of this road. Null when
	 *         {@link CSURoadHelper#getPfClearLine(CSURoadHelper)} has not been invoked
	 *         somewhere before this metthod or the return value of
	 *         {@link CSURoadHelper#getPfClearLine(CSURoadHelper)} is null.
	 */
	...
	/*
	 * public boolean hasIntersection(Polygon polygon,
	 * rescuecore2.misc.geometry.Line2D line) {
	 * List<rescuecore2.misc.geometry.Line2D> polyLines = getLines(polygon); for
	 * (rescuecore2.misc.geometry.Line2D ln : polyLines) { Point2D p =
	 * GeometryTools2D.getSegmentIntersectionPoint(ln, line); if (p != null)
	...
	private List<rescuecore2.misc.geometry.Line2D> getLines(Polygon polygon) {
		List<rescuecore2.misc.geometry.Line2D> lines = new LinkedList<>();
		int count = polygon.npoints;
		for (int i = 0; i < count; i++) {
			int j = (i + 1) % count;
			rescuecore2.misc.geometry.Point2D p1 = new rescuecore2.misc.geometry.Point2D(polygon.xpoints[i],
					polygon.ypoints[i]);
			rescuecore2.misc.geometry.Point2D p2 = new rescuecore2.misc.geometry.Point2D(polygon.xpoints[j],
					polygon.ypoints[j]);
