package CSU_Yunlu_2020.module.route.pov.graph;
...
import CSU_Yunlu_2020.util.IdSorter;
...
class POVBuilder {
	...
	// constructor
	POVBuilder(final WorldInfo world) {
		List<PointNode> basePoints = createBasics(world);
		points = new PointNode[basePoints.size()];
		idToArea = new HashMap<EntityID, AreaNode>(getEntitiesOfType(world, AREAS).size());
	...
	public Collection<StandardEntity> getEntitiesOfType(WorldInfo world, EnumSet<StandardEntityURN> urns) {
	...
	private List<PointNode> createBasics(final WorldInfo world) {
		...
		ArrayList<StandardEntity> areas = new ArrayList<>(getEntitiesOfType(world, AREAS));
