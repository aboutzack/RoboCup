...
import CSU_Yunlu_2019.module.route.pov.graph.PassableDictionary.PassableLevel;
import CSU_Yunlu_2019.module.route.pov.graph.PointOfVisivility;
import CSU_Yunlu_2019.module.route.pov.reachable.UFTReachableArea;
...
public class POVRouter {
	...
	public POVRouter(AgentInfo agent, WorldInfo world, ScenarioInfo scenario,Map<EntityID, CSURoadHelper> csuRoadMap) {
		// public POVRouter(Human me, AdvancedWorldModel world) {
		this.world = world;
		this.scenario = scenario;
		this.me = (Human) world.getEntity(agent.getID());

		pov = new PointOfVisivility(world);
		this.csuRoadMap=csuRoadMap;
		
		passableDic = new PassableDictionary(agent, world, scenario,csuRoadMap);
		// bfsReachable = new BFSReachableArea(world);
		uftReachable = new UFTReachableArea(world);

		final int routeThinkTime = scenario.getKernelAgentsThinkTime() * 2 / 3;
	...
	public void update(AgentInfo agentInfo, WorldInfo worldInfo, ScenarioInfo scenarioInfo, final EntityID pos,
		...
		if (uftReachable != null) {
			uftReachable.update(agentInfo, worldInfo, scenarioInfo, this, newPassables);
		}
	}
	...
	public List<EntityID> getMultiDest(Area origin, Collection<? extends StandardEntity> destinations, Point start) {
		return getMultiAStar(origin, destinations, this.getNormalCostFunction(), start);
	}

	public List<EntityID> getMultiDest(Area origin, Collection<? extends StandardEntity> destinations,
			CostFunction costFunc, Point start) {
		return getMultiAStar(origin, destinations, costFunc, start);
		// return getDijkstra(origin, new HashSet<StandardEntity>(destinations),
		// costFunc);
	}

	// unused path searcher
	public List<EntityID> getDijkstra(Area origin, Set<StandardEntity> destinations, CostFunction costFunc,
			Point start) {
	...
	public List<EntityID> getMultiAStar(Area origin, Collection<? extends StandardEntity> destinations,
			CostFunction costFunc, Point start) {
	...
	public CostFunction getFbCostFunction(Building dest) {
		return CostFunctionFactory.fb(scenario, passableDic, dest);
	}
	...
	public POVPath amend(Human me, POVPath old) {
		...
		return search.getAStarPath(me, (Area) world.getEntity(old.getRoute().get(0)), getNormalCostFunction()).add(old);
	}
