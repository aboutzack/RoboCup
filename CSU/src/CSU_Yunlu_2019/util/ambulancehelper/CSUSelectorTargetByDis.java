package CSU_Yunlu_2019.util.ambulancehelper;
...
//        }
    ...
    private CSUTarget findBestVictim(Map<EntityID, CSUTarget> targetsMap, Collection<StandardEntity> elements) {
        CSUTarget bestTarget = null;
...
//                        minValue = target.getCost();
        ...
        if (victims != null && !victims.isEmpty()) {
            for (StandardEntity victim : victims) {
                ...
                if (target == null) {
                    ...
                    target.setDistanceToPartition(CSUSelectorTargetByDis.getDistance.distance(victim.getLocation(worldInfo.getRawWorld()), clusterCenterMap.get(this.clusterIndex)));
                }
    ...
    private int computingDistance(Human human) {
        ...
        return (int) (coefficient * CSUSelectorTargetByDis.getDistance.distance(human.getPosition(worldInfo.getRawWorld()).getLocation(worldInfo.getRawWorld()), worldInfo.getLocation(agentInfo.me())));
    }
