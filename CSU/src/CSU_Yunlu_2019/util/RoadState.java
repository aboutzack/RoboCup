...
public class RoadState {
	...
	public static boolean isSmoothRoad(Road road, WorldInfo world) {
	...
	public static boolean isBlockingRoad(Blockade blockade, WorldInfo world) {
		...
		for (Edge roadEdge : road.getEdges()) {
			if (roadEdge.isPassable()) {
				for (int i = 0; i < blockadeVertices.length; i += 2) {
					if (!openRoadEdge(blockade.getID(), world, road, roadEdge, blockadeVertices)) {
						return true;
					}
				}
			}
		...
		if (otherBlockades != null && otherBlockades.size() > 1) {
			for (EntityID otherBlockadeID : otherBlockades) {
				if (otherBlockadeID.getValue() != blockade.getID().getValue()) {
					...
					if (entity == null)
						continue;
					int[] otherBlockadeVertices = ((Blockade) entity).getApexes();
					for (int j = 0; j < otherBlockadeVertices.length; j += 2) {
						for (int i = 0; i < blockadeVertices.length; i += 2) {
							Line2D blockadeLine = new Line2D(new Point2D(blockadeVertices[i], blockadeVertices[i + 1]),
									new Point2D(blockadeVertices[(i + 2) % blockadeVertices.length],
											blockadeVertices[(i + 3) % blockadeVertices.length]));
	...
	public static boolean openRoadEdge(EntityID blockadeID, WorldInfo model, Road road, Edge roadEdge,
		...
		LinkedList<Point2D> projectedPoints = new LinkedList<Point2D>();
		...
		if (minDistanceEnd > 2000 || minDistanceStart > 2000) {
			...
			projectedPoints = new LinkedList<Point2D>();
			for (int i = 0; i < blockadeVertices.length; i += 2) {
	...
	public static Edge getNeighbourEdge(WorldInfo model, Road road, Edge roadEdge) {
