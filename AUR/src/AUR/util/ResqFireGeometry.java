...
public class ResqFireGeometry {
	...
	public static Point getRndPoint(Point a, Point b){		
		float[] mb=ResqFireGeometry.getAffineFunction((float)a.x,(float)a.y,(float)b.x,(float)b.y);		
		float dx=(Math.max((float)a.x,(float)b.x)-Math.min((float)a.x,(float)b.x));
		dx*=ResqFireRnd.get01();
	...
	public static Point getRndPoint(Point a, double length){		
		double angel=ResqFireRnd.get01()*2d*Math.PI;
